export function mitmbfs(grid) {
    ...
    let vid = [generateFrame(grid, rows, cols, null, 'Starting meet-in-the-middle bfs algorithm...', [])];
    ...
    while (queue.length > 0) {
      ...
      vid.push(generateFrame(grid, rows, cols, null, 'Removed node from queue', []));
        if ((grid[searchNode.row][searchNode.column].fromStart && grid[searchNode.row + 1][searchNode.column].fromEnd) ||
            (grid[searchNode.row][searchNode.column].fromEnd && grid[searchNode.row + 1][searchNode.column].fromStart)){
                let path = calcPath(grid[searchNode.row][searchNode.column], grid[searchNode.row + 1][searchNode.column]);
                console.log(path);
            }
        else if (
          //Search Up
